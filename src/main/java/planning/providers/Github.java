...
public class Github implements InjectableProvider<Auth, Parameter> {
  ...
  @Override
  public Injectable<GitHubClient> getInjectable(
      final ComponentContext ic, final Auth a, final Parameter c) {
  ...
  private final class GithubInjectable extends AbstractHttpContextInjectable<GitHubClient> {
    ...
    public GitHubClient getValue(final HttpContext c) {
      ...
      if (token.isPresent()) {
        final GitHubClient client =
            new GitHubClient(
                (IGitHubConstants.HOST_DEFAULT.equals(Github.this.githubHostname) ? "api." : "")
                    + Github.this.githubHostname);
    ...
    private Optional<String> readToken(final HttpSession session) {
      return (session == null)
          ? Optional.absent()
          : Optional.fromNullable((String) session.getAttribute("token"));
    }
  }
}
